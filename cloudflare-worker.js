  ...
  async fetch(request, env, ctx) {
    ...
    if (pathname === '/api/narrative' && request.method === 'POST') {
      try {
        ...
        const rules = lang === 'en'
          ? `Rules:\n1. Use ONLY actual disclosed data in the context. IGNORE any forecast records.\n2. ${topicHint || 'Match the user\'s specific topic to the right metric in the context.'}\n3. Strict output format (follow exactly):\n   - Sentence 1: Directly address the user\'s premise in one short sentence.\n   - One bullet: "- [Metric] YoY slowed from **+[prior]%** in [Prior Holiday] to **+[current]%** in [Current Holiday], reaching **[volume]**. Source: [URL]"\n   - Sentence 2: One short macro anchor based ONLY on data in the context.\n   Keep total answer to 2 short sentences + 1 bullet. No extra paragraphs.\n4. Do not insert manual line breaks inside sentences; let text flow naturally.\n5. Use the readable units already converted in the context (million/billion trips, billion CNY). NEVER output raw units like "10,000 trips" or "100M trips".\n6. Bold ALL key numbers (YoY % and volume) with **number**.\n7. Cite the source URL once at the end of the bullet.\n8. Holiday names must be in English (e.g. Dragon Boat, Labour Day). Never output Chinese characters.\n9. The macro anchor sentence must use only data in the context. Do NOT mention pandemic, recovery, government policy, or any trend not explicitly supported by the numbers.\n10. If the user asks about train/railway, use MOT railway volume and railway_yoy as the primary metric. Never use total cross-region flow as a proxy.\n11. If the metric is missing or null, say exactly: "The context does not contain [metric] data for the requested period."\n12. Do not invent numbers or macro trends.`
        ...
        const userPrompt = lang === 'en'
          ? `Official data context:\n${context}\n\nUser question: ${question}\n\nPlease answer in English. Remember: only use numbers from the context; if railway/aviation/inbound/etc. data is not in the context, say it is not available.`
