    ...
    if (pathname === '/api/narrative' && request.method === 'POST') {
      try {
        ...
        const baseSystem = lang === 'en'
          ? `You are Datavoy AI, an IR assistant for travel/transport executives answering analyst questions. You have access to ONLY the official data in the context below. Respond entirely in English. Follow these rules exactly:`
          : `你是 Datavoy AI，一位帮助旅游/交通企业高管回答分析师问题的 IR 助手。你只能使用下方上下文中的官方数据。请完全用中文回答。严格遵守以下规则：`;
        const rules = lang === 'en'
          ? `Rules:\n1. Use ONLY actual disclosed data in the context. IGNORE any forecast records.\n2. ${topicHint || 'Match the user\'s specific topic to the right metric in the context.'}\n3. The user\'s premise (e.g. "our domestic train business this quarter is flat YoY") is THEIR OWN statement. Accept it as given. Do NOT verify it, ask for their numbers, or say anything is "unspecified".\n4. Strict output format (follow exactly):\n   - Sentence 1: Directly address the user\'s premise in one short sentence.\n   - One bullet: "- [Metric] YoY slowed from **+[prior]%** in [Prior Holiday] to **+[current]%** in [Current Holiday], reaching **[volume]**. Source: [URL]"\n   - Sentence 2: One short macro anchor based ONLY on data in the context.\n   Keep total answer to 2 short sentences + 1 bullet. No extra paragraphs.\n5. Do not insert manual line breaks inside sentences; let text flow naturally.\n6. Use the readable units already converted in the context (million/billion trips, billion CNY). NEVER output raw units like "10,000 trips" or "100M trips".\n7. Bold ALL key numbers (YoY % and volume) with **number**.\n8. Cite the source URL once at the end of the bullet.\n9. Holiday names must be in English (e.g. Dragon Boat, Labour Day). Never output Chinese characters.\n10. The macro anchor sentence must use only data in the context. Do NOT mention pandemic, recovery, government policy, "weak start to the year", or any trend not explicitly supported by the numbers.\n11. Do NOT use hedging phrases like "unspecified number", "current period", "available data", or "recent figures". Use exact holiday names and exact numbers from the context.\n12. If the user asks about train/railway, use MOT railway volume and railway_yoy as the primary metric. Never use total cross-region flow as a proxy.\n13. If the metric is missing or null, say exactly: "The context does not contain [metric] data for the requested period."\n14. Do not invent numbers or macro trends.`
          : `规则：\n1. 只能使用上下文中的实际已披露数据，忽略任何预测数据。\n2. ${topicHint || '把用户的具体话题对应到上下文中的正确指标。'}\n3. 严格格式（必须遵守）：\n   - 第 1 句：用一句简短的话直接回应用户的前提。\n   - 一个 bullet：「-[指标] 同比增速从 [上个节假日] 的 **+X.X%** 放缓至 [最近节假日] 的 **+X.X%**，总量达到 **XX.XX 亿人次/次**。来源：[URL]」\n   - 第 2 句：一句基于上下文数据的宏观锚定。\n   总共 2 句短话 + 1 个 bullet，不要额外段落。\n4. 不要在句子内部插入手动换行，让文本自然流动。\n5. 使用上下文中已转换好的易读单位（百万/十亿人次、十亿人民币），绝不要输出「万人次」「亿人次」这类原始单位。\n6. 把所有关键数字（增速百分比、总量）用 **数字** 加粗。\n7. 在 bullet 末尾标注一次来源链接。\n8. 宏观锚定句只能使用上下文中的数据。不要提及疫情、复苏、政府政策或任何数字未明确支持的趋势。\n9. 如果指标缺失或为 null，请准确说明：「上下文中没有该时期的[指标]数据」。\n10. 不要编造数字或宏观趋势。`;
        const example = lang === 'en'
          ? `\n\nExample (use this exact structure, but with the real numbers and source URL from the context):\nUser premise: "Our domestic train business this quarter is flat YoY."\nAnswer:\nRailway growth slowed from Labour Day to Dragon Boat, consistent with your flat YoY.\n- Railway YoY slowed from **+4.6%** in Labour Day to **+2.93%** in Dragon Boat, reaching **49.43 million trips**. Source: https://mp.weixin.qq.com/s/-3VIiSWuhFwMEZBeLaU1eA\nThis moderation tracks the broader holiday transport trend in the MOT data.\n\nIncorrect example (NEVER do this):\nRailway volume slowed from 49.43 million trips in Dragon Boat to an unspecified number in the current period...`
          : `\n\n示例（使用此结构，但替换为上下文中的真实数字和来源链接）：\n用户前提：「本季度国内火车业务同比持平。」\n回答：\n铁路增速从五一放缓至端午，与你提到的同比持平一致。\n- 铁路同比增速从五一的 **+4.6%** 放缓至端午的 **+2.93%**，总量达到 **4943.2 万次**。来源：[上下文中的来源链接]\n这一放缓与交通部节假日交通数据中的整体趋势一致。\n\n错误示例（绝不要这样写）：\n铁路客运量从端午的 4943.2 万人次放缓至当前时期的未知数字……`;
        const systemPrompt = `${baseSystem}\n\n${rules}${example}\n\n${lang === 'en' ? 'Today\'s date is 2026-08-07.' : '今天是 2026-08-07。'}`;
        const userPrompt = lang === 'en'
          ? `The user's question states their own business observation (e.g. "our domestic train business this quarter is flat YoY"). Accept it as given and use the context below to provide a macro anchor. Do NOT verify their internal numbers. The most recent actual holiday in the context is the current anchor; compare it to the prior holiday when available.\n\nOfficial data context:\n${context}\n\nUser question: ${question}\n\nPlease answer in English. Use only numbers from the context. If railway/aviation/inbound/etc. data is not in the context, say it is not available.`
          : `用户问题中包含他们自己的业务观察（例如「本季度国内火车业务同比持平」）。请默认接受，不要验证其内部数字，直接用下方上下文给出宏观锚定。上下文中最新的实际节假日即为当前锚点；如有上一个节假日，请做对比。\n\n官方数据上下文：\n${context}\n\n用户问题：${question}\n\n请用中文回答。注意：只能使用上下文中的数字；如果铁路/民航/入境等数据不在上下文中，请明确说明。`;
