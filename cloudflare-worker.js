  ...
  async fetch(request, env, ctx) {
    ...
    if (pathname === '/api/narrative' && request.method === 'POST') {
      try {
        ...
        const rules = lang === 'en'
          ? `Rules:\n1. Use ONLY actual disclosed data in the context. IGNORE any forecast records.\n2. ${topicHint || 'Match the user\'s specific topic to the right metric in the context.'}\n3. Start your answer by directly stating the relevant figure (e.g. railway YoY / aviation YoY / inbound foreigner YoY) from the matching data source.\n4. Use ONLY numbers that appear in the context. If the metric is missing or null, say exactly: "The context does not contain [metric] data for the requested period."\n5. Keep the answer to 3–5 bullets plus one short macro anchor sentence.\n6. Bold the key takeaway.\n7. Cite the source URL for every figure.\n8. Do not invent macro trends or figures. Do not write generic economic essays.`
          : `规则：\n1. 只能使用上下文中的实际已披露数据，忽略任何预测数据。\n2. ${topicHint || '把用户的具体话题对应到上下文中的正确指标。'}\n3. 回答必须先直接给出相关指标数字（例如 railway YoY / aviation YoY / 外国人 YoY）。\n4. 只能使用上下文中出现的数字。如果指标缺失或为 null，请准确说明：「上下文中没有该时期的[指标]数据」。\n5. 回答控制在 3–5 个 bullet + 一句简短宏观锚定。\n6. 加粗核心结论。\n7. 每个数字都要标注来源链接。\n8. 不要编造宏观趋势或数字，不要写泛泛的经济分析。`;
