  ...
  async fetch(request, env, ctx) {
    ...
    if (pathname === '/api/narrative' && request.method === 'POST') {
      try {
        ...
        const rules = lang === 'en'
          ? `Rules:\n1. Use ONLY actual disclosed data in the context. IGNORE any forecast records.\n2. ${topicHint || 'Match the user\'s specific topic to the right metric in the context.'}\n3. Format: 1 short opening sentence, then 1 bullet with the key figure, then 1 short macro anchor sentence. Total 2–3 sentences + 1 bullet. Do not insert manual line breaks inside sentences; let text flow naturally.\n4. Start with the relevant metric and compare it to the prior holiday in the context when available (e.g. "Railway YoY slowed from +4.6% in Labour Day to +2.93% in Dragon Boat").\n5. Use the readable units already converted in the context (million/billion trips, billion CNY). NEVER output raw units like "10,000 trips" or "100M trips".\n6. Bold ALL key numbers (YoY % and volume) with **number**.\n7. Cite the source URL once at the end of the bullet.\n8. Holiday names must be in English (e.g. Dragon Boat, Labour Day). Never output Chinese characters.\n9. If the user asks about train/railway, use MOT railway volume and railway_yoy as the primary metric. Never use total cross-region flow as a proxy.\n10. If the metric is missing or null, say exactly: "The context does not contain [metric] data for the requested period."\n11. Do not invent numbers or macro trends.`
          : `规则：\n1. 只能使用上下文中的实际已披露数据，忽略任何预测数据。\n2. ${topicHint || '把用户的具体话题对应到上下文中的正确指标。'}\n3. 格式：1 句开场 + 1 个关键数字 bullet + 1 句宏观锚定。总共 2–3 句话 + 1 个 bullet。不要在句子内部插入手动换行，让文本自然流动。\n4. 优先把最新节假日和上下文中的上一个节假日做对比（例如「铁路同比增速从五一的+4.6%放缓至端午的+2.93%」）。\n5. 使用上下文中已转换好的易读单位（百万/十亿人次、十亿人民币），绝不要输出「万人次」「亿人次」这类原始单位。\n6. 把所有关键数字（增速百分比、总量）用 **数字** 加粗。\n7. 在 bullet 末尾标注一次来源链接。\n8. 如果指标缺失或为 null，请准确说明：「上下文中没有该时期的[指标]数据」。\n9. 不要编造数字或宏观趋势。`;
